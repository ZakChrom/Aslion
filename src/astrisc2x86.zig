...
const Arg = enum(u2) { NO, ARG, IMMEDIATE };

const HashMapData = struct { replacement: []const u8, arg: Arg };

pub fn convert(astrisc: []u8) !void {
    var s = std.StringHashMap(HashMapData).init(std.heap.page_allocator);
    const put = struct {
        fn _(h: *std.StringHashMap(HashMapData), k: []const u8, r: []const u8, a: Arg) !void {
